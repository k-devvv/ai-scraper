 ...
 *  7. Proxy slot          — residential proxy ready to wire in
 */
...
// Injects ±1 pixel noise into canvas readback and spoofs WebGL renderer string.
// Each session gets a unique noise seed so every instance has a different fingerprint.

function buildFingerprintScript(noiseSeed: number): string {
  return `
(function() {
...
export async function fetchPage(
  ...
  const context = await browser.newContext({
    ...
    extraHTTPHeaders: {
      // Synced to the claimed Chrome version — critical for Cloudflare
  ...
  await page.addInitScript(buildFingerprintScript(noiseSeed));
